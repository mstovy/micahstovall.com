...
export default function Street() {
  // Load all images in the street folder using Vite import.meta.glob
  const modules = import.meta.glob("../../assets/images/street/*.{jpg,jpeg,png,webp}", {
    eager: true,
    as: "url",
  }) as ImageMap
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 py-12">
      <h1 className="mb-6 text-3xl font-semibold text-slate-900 dark:text-slate-100">Street Gallery</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {images.length === 0 && (
          <div className="col-span-full rounded-lg border border-dashed border-slate-200 p-8 text-center text-sm text-slate-600">
            No images found in <span className="font-mono">src/assets/images/street</span>
          </div>
        )}
        ...
        {images.map((src, i) => (
          ...
          >
            <img src={src} alt={`Street ${i + 1}`} className="w-full h-48 object-cover transition-transform duration-300 hover:scale-105" />
