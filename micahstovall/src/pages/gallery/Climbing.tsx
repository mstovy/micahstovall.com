...
export default function Climbing() {
  ...
  const modules = import.meta.glob("../../assets/images/climbing/*.{jpg,jpeg,png,webp}", {
    eager: true,
    as: "url",
  }) as ImageMap
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {images.length === 0 && (
          <div className="col-span-full rounded-lg border border-dashed border-slate-200 p-8 text-center text-sm text-slate-600">
            No images found in <span className="font-mono">src/assets/images/climbing</span>
          </div>
        )}
        ...
        {images.map((src, i) => (
          ...
          >
            <img src={src} alt={`Climbing ${i + 1}`} className="w-full h-48 object-cover transition-transform duration-300 hover:scale-105" />
