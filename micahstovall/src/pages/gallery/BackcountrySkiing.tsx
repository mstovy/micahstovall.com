...
export default function BackcountrySkiing() {
  // Load all images in the backcountryskiing folder using Vite import.meta.glob
  const modules = import.meta.glob("../../assets/images/backcountryskiing/*.{jpg,jpeg,png,webp}", {
    eager: true,
    as: "url",
  }) as ImageMap
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {images.length === 0 && (
          <div className="col-span-full rounded-lg border border-dashed border-slate-200 p-8 text-center text-sm text-slate-600">
            No images found in <span className="font-mono">src/assets/images/backcountryskiing</span>
          </div>
        )}
        ...
        {images.map((src, i) => (
          ...
          >
            <img src={src} alt={`Backcountry Skiing ${i + 1}`} className="w-full h-48 object-cover transition-transform duration-300 hover:scale-105" />
        ...
        >
          <div className="relative max-h-[90vh] w-full max-w-4xl">
            ...
            <div className="flex items-center justify-center overflow-hidden rounded">
              <img
                src={images[index]}
                alt={`Backcountry Skiing ${index + 1}`}
